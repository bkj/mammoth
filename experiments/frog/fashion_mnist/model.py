...
sys.path.append('/home/bjohnson/projects/frog/frog')
...
class Architecture(FROGArchitecture):
    def __init__(self, num_nodes, normal=None, num_prev=2, scale=1e-3):
        ...
        self._arch_params = [self.normal]
        
    def __repr__(self):
        return 'FashionMNISTArchitecture(num_nodes=%d | num_ops=%d)' % (self.num_nodes, self.num_ops)
...
class Network(FROGSearchMixin, BaseNet):
  def __init__(self, in_channels, op_channels, num_classes, num_layers, num_nodes=4, cat_last=1, stem_multiplier=1, num_inputs=2):
    super().__init__(loss_fn=F.cross_entropy)
